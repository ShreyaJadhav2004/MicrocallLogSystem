...
{
	if(empty($productname) || empty($customername) || empty($customermobile) || empty($problemdetails))
	{
	...
	{
		$date=date('Y-m-d');
		$sql="INSERT INTO table_generateticket(productname, customername, customermobile, problemdetails, solutiondetails, ticketgenrationdate) VALUES ('$productname', '$customername', '$customermobile', '$problemdetails', 'Pending', '$date')";
            ...
            <div class="col-sm-8 order-sm-2">
                ...
                <form method="POST" action="generateticket.php">
					...
					if(isset($_POST['submit']))
					{
						if(empty($lastid))
						{
							echo "Thank for registering your complaint. Your ticket id=";
						}
						else
						{
